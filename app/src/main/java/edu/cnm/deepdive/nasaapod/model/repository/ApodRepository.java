...
public class ApodRepository {
  ...
  private static final String MEDIA_RECORD_FAILURE = "Unable to create Media store record.";
  ...
  public Completable downloadImage(@NonNull Apod apod) {
    ...
    return nasa.getFile(url)
        ...
        .map((body) -> {
          ...
          Uri uri = getMedaiUri(reslover, url, apod.getTitle());
          try (
              InputStream input = body.byteStream();
              OutputStream output = reslover.openOutputStream(uri);
              ) {
            copy(input, output);
          } catch (IOException ex) {
  ...
  private Uri getMedaiUri(@NonNull ContentResolver resolver, @NonNull String sourceUrl,
